...
目前是在Windows平台下进行编码、编译及演示。Windows平台测试通过。
本文档需要完善，目前只写了粗略概要.
...
1.2 demo_my_c_lib 
对 demo_third_lib 生成的开源库的封装, 并加入适当的业务逻辑. 比如调用libevent接口自动重连服务器的功能逻辑. 再比如报文的编解码等业务实现. 同样, 本项目需要调用 demo_third_lib 的库, 生产特定平台的库(本文生成静态库).

1.3 demo_android_sdk 
对 demo_my_c_lib 提供的接口用java接口来转换,使开发者方便的用于安卓项目中. 这类项目通常是一个普通的jni封装,使c接口变为java接口. 本项目中演示了把底层c接口变换后, 生成一个jar包及动态库的方式.
